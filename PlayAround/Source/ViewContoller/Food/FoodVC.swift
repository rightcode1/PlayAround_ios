...
class FoodVC: BaseViewController, FoodCategoryReusableViewDelegate, SelectVillage {
    func select() {
    }
    ...
    func initFoodList(sort: FoodSort? = nil) {
        self.showHUD()
        let param = FoodListRequest(category: category == .전체 ? nil : category,villageId: DataHelperTool.villageId, sort: sort == nil ? .최신순 : sort )
