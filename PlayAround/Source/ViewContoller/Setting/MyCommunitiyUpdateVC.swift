...
class MyCommunitiyUpdateVC: BaseViewController,ViewControllerFromStoryboard, reloadPermit{
    func reload() {
        initCommunityList()
    }
    ...
    @IBOutlet weak var contentUpdateTextField: UITextField!
    
    
    var communityId: Int = 0
    
    var data: CommuintyList?{
        didSet{
            imageList = data?.images ?? []
            switch data?.category{
            case "아파트별모임":
                selectedCategory = .아파트별모임
                break
            case "스터디그룹":
                selectedCategory = .스터디그룹
                break
            case "동호회":
                selectedCategory = .동호회
                break
            case "맘카페":
                selectedCategory = .맘카페
                break
            case "기타":
                selectedCategory = .기타
                break
            ...
            selectVillageList = data?.address ?? ""
            selectVillageId = data?.villageId
        }
    }
    ...
    func updateData(){
        ...
        for count in 0 ..< villageList.count{
            if villageList[count].address == selectVillageList{
                selectIndex = count
                villageList[count].isselect = true
            }
        }
        mainTableView.reloadData()
        nameUpdateTextField.text = data?.name
        contentUpdateTextField.text = data?.content
    }
    ...
    func deleteImage(index: Int) {
        ...
        APIProvider.shared.communityAPI.rx.request(.imageRemove(id: [imageList[index].id]))
                    .filterSuccessfulStatusCodes()
                    .subscribe(onSuccess: { response in
    ...
    func registerCommunity(){
      ...
      if nameUpdateTextField.text!.isEmpty {
        callOkActionMSGDialog(message: "커뮤니티명을 입력해주세요."){
        }
        return
      }
      if contentUpdateTextField.text!.isEmpty {
        callOkActionMSGDialog(message: "한줄 설명을 입력해주세요."){
        }
        return
      }
      if selectVillageList == nil {
        callOkActionMSGDialog(message: "동네를 선택해주세요."){
        }
        return
      }
      self.showHUD()
      APIProvider.shared.communityAPI.rx.request(.CommuntyUpdate(id: communityId, param: RegistCommunityRegister(isSecret: selectSecret, name: nameUpdateTextField.text!, content: contentUpdateTextField.text!, villageId: selectVillageId!, category: selectedCategory.rawValue)))
        .filterSuccessfulStatusCodes()
        ...
        tapOn.rx.tapGesture().when(.recognized)
            .bind(onNext: { [weak self] _ in
                ...
                if self.tapOn.backgroundColor == #colorLiteral(red: 0.9686275125, green: 0.9686275125, blue: 0.9686276317, alpha: 1){
                    ...
                    self.tapOff.backgroundColor = #colorLiteral(red: 0.9686275125, green: 0.9686275125, blue: 0.9686276317, alpha: 1)
                    self.tapOff.tintColor = #colorLiteral(red: 0.5607842803, green: 0.5607842803, blue: 0.5607843399, alpha: 1)
                    self.selectSecret = true
                }else{
                    self.tapOn.backgroundColor = #colorLiteral(red: 0.9686275125, green: 0.9686275125, blue: 0.9686276317, alpha: 1)
                    self.tapOn.tintColor = #colorLiteral(red: 0.5607842803, green: 0.5607842803, blue: 0.5607843399, alpha: 1)
                    self.tapOff.backgroundColor = #colorLiteral(red: 0.9704249501, green: 0.5212578773, blue: 0.1708718836, alpha: 1)
                    self.tapOff.tintColor = .white
                    self.selectSecret = false
